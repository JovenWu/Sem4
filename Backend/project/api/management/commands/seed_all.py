...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            # Step 1: Seed Products and Categories
            if not options['skip_products']:
                self.stdout.write('\n' + '='*50)
                self.stdout.write(self.style.SUCCESS('STEP 1: Seeding Products and Categories'))
                self.stdout.write('='*50)
            ...
            # Step 2: Seed Purchase Orders (this will also update stock levels)
            if not options['skip_purchase_orders']:
                self.stdout.write('\n' + '='*50)
                self.stdout.write(self.style.SUCCESS('STEP 2: Seeding Purchase Orders'))
                self.stdout.write('='*50)
            ...
            # Step 3: Seed Sales Records (this will reduce stock levels)
            if not options['skip_sales']:
                self.stdout.write('\n' + '='*50)
                self.stdout.write(self.style.SUCCESS('STEP 3: Seeding Sales Records'))
                self.stdout.write('='*50)
    ...
    def clean_existing_data(self):
        ...
        from api.models import SalesRecords, PurchaseOrderItems, PurchaseOrders, Products, Categories
    ...
    def display_summary(self):
        ...
        try:
            from api.models import Categories, Products, PurchaseOrders, PurchaseOrderItems, SalesRecords
